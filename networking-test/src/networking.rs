...
pub struct IPs {
    ...
    #[serde(rename = "turtlebot_ips")] //Rename the turtlebot_ips in the json to ip_addresses. Done to keep name. Overcomplicating things be like:
    pub ip_addresses: Vec<String>, // array of ip strings
    #[serde(rename = "my_smelly_key")]
...
async fn main() {
    ...
    let file = File::open("../data/data.json"); // change the path if needed
    let file = match file {
        Ok(file) => file,
        Err(e) => {
            println!("{:?}", e);
